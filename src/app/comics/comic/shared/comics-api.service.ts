...
export class ComicsApiService {
  ...
  getCharactersComic (comicId:string): Observable<ComicsComponent>{
    const url_comicDetail = 'https://gateway.marvel.com:443/v1/public/comics';
    const url_comicId = comicId;
    const url_key = 'characters?ts=1&apikey=1607336a86d386d6990a2518297c8e73&hash=4ba62a449c1e73ddd105d99bb5279dc1';
    const url =  `${url_comicDetail}/${url_comicId}/${url_key}`;
    return this.http.get<ComicsComponent>(url)
    .pipe(map((data:any) => data.data.results)
    ) 
  }
  getComicCreator (comicId:string): Observable<ComicsComponent>{
    const url_comicDetail = 'https://gateway.marvel.com:443/v1/public/comics';
    const url_comicId = comicId;
    const url_key = 'creators?ts=1&apikey=1607336a86d386d6990a2518297c8e73&hash=4ba62a449c1e73ddd105d99bb5279dc1';
    const url =  `${url_comicDetail}/${url_comicId}/${url_key}`;
    return this.http.get<ComicsComponent>(url)
    .pipe(map((data:any) => data.data.results)
    ) 
